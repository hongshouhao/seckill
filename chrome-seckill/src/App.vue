<template>
  <div id="app"
    ...
    <Checkbox v-model="ifUseLocalTime"
              @change="switchTime">本地时间</Checkbox>
    <Button v-show="ifUseLocalTime===false"
            type="primary"
            size="small"
            @click="useServerTime">校准</Button>
...
export default {
  ...
  methods: {
    ...
    updateTime () {
      ...
      let timer = setInterval(function () {
        ...
        for (let i = 0; i < _this.taskList.length; i++) {
          ...
          taskItem.leftTime = _this.getLeftTime(taskItem.killTime - _this.standerTime + 1000)
        }
      }, 1000)
    ...
    useServerTime () {
      let _this = this
      // 北京时间接口路径
      let apiUrl = 'https://sapi.k780.com/?app=life.time&appkey=10003&sign=b59bc3ef6191eb9f747dd4e83c99f2a4&format=json&jsoncallback=data'
      // 调用接口获取北京时间
    ...
    loadTasks () {
      let _this = this
      axios.get('static/tasks.json').then(function (response) {
