  ...
  chrome.storage.local.get({ tasks: new Array() }, function (value) {
    ...
    if (tasks.length > 0) {
      ...
      for (var i = 0; i < tasks.length; i++) {
        ...
        // 重置标题
        $(card)
          .find("span[datafld='name']")
          .text(
            tasks[i].name.length > titleLength
              ? tasks[i].name.substr(0, titleLength) + '...'
              : tasks[i].name
          )
        $(card)
          .find("span[datafld='name']")
          .attr('taskId', tasks[i].id)
        $(card)
          .find("span[datafld='name']")
          .attr('title', tasks[i].name)
        $(card)
          .find("span[datafld='url']")
          .text(
            tasks[i].url.length > urlLength
              ? tasks[i].url.substr(0, urlLength) + '...'
              : tasks[i].url
          )
        $(card)
          .find("span[datafld='url']")
          .attr('title', tasks[i].url)
        $(card)
          .find("span[datafld='selector']")
          .text(tasks[i].selector)
        $(card)
          .find("span[datafld='location']")
          .text(
            tasks[i].location.length > urlLength
              ? tasks[i].location.substr(0, urlLength) + '...'
              : tasks[i].location
          )
        $(card)
          .find("span[datafld='location']")
          .attr('title', tasks[i].location)
        $(card)
          .find("span[datafld='killTime']")
          .text(tasks[i].killTime.replace('T', ' '))
        $(card)
          .find("span[datafld='leftTime']")
          .text(
            getLeftTime(new Date(tasks[i].killTime).getTime() - standerTime)
          )
        $(card)
          .find("span[datafld='leftTime']")
          .attr('killTime', new Date(tasks[i].killTime).getTime())
        $(card)
          .find("span[datafld='frequency']")
          .text(tasks[i].frequency)
        $(card)
          .find("span[datafld='count']")
          .text(tasks[i].count)
        $(card)
          .find('.footer ul')
          .attr('id', tasks[i].id)
  ...
  $(document).delegate('.footer ul li', 'click', function () {
    ...
    if ($(this).index() == 3) {
      var contentDiv = $(this)
        ...
        .prev()
      if ($(this).text() == '保存') {
        currentTask.killTime = $(contentDiv)
          .find("input[name='killTime']")
          .val()
        currentTask.frequency = $(contentDiv)
          .find("input[name='frequency']")
          .val()
        currentTask.count = $(contentDiv)
        ...
        $(contentDiv)
          .find("span[datafld='killTime']")
          .text(currentTask.killTime.replace('T', ' '))
        $(contentDiv)
          .find("span[datafld='leftTime']")
          .text(
            getLeftTime(new Date(currentTask.killTime).getTime() - standerTime)
          )
        $(contentDiv)
          .find("span[datafld='leftTime']")
          .attr('killTime', new Date(currentTask.killTime).getTime())
        $(contentDiv)
          .find("span[datafld='frequency']")
          .text(currentTask.frequency)
        $(contentDiv)
          .find("span[datafld='count']")
          .text(currentTask.count)
        $(contentDiv)
          .find('.update')
          .css('display', 'inline')
        $(contentDiv)
        ...
        if (currentTask.status == 0) {
          ...
          $(contentDiv)
            .find("input[name='killTime']")
            .val(formatDate(new Date().getTime()) + 'T12:00:01')
          $(contentDiv)
            .find("input[name='frequency']")
            .val(currentTask.frequency)
          $(contentDiv)
            .find("input[name='count']")
            .val(currentTask.count)
          $(this).text('保存')
          $(this).css('color', 'blue')
        }
      }
...
function updateTime (standerTime) {
  var timer = setInterval(function () {
    ...
    $("span[datafld='leftTime']").each(function () {
      $(this).text(
        getLeftTime(parseInt($(this).attr('killTime')) - standerTime + 1000)
      )
    })
  }, 1000)
  if (oldTimer != null) {
